...
void Recover_state_columnwise( unsigned char known_diff, unsigned char pos, unsigned char count, unsigned char **ptr ) {
	...
	for( i = 0; i < 256; ++i ) {
		...
		if( ( ( s[ i ] ^ s[ i ^ 0x01 ] ) == diff[ 0 ] ) || ( ( s[ i ] ^ s[ i ^ 0x02 ] ) == diff[ 0 ] ) || ( ( s[ i ] ^ s[ i ^ 0x04 ] ) == diff[ 0 ] ) || ( ( s[ i ] ^ s[ i ^ 0x08 ] ) == diff[ 0 ] ) || ( ( s[ i ] ^ s[ i ^ 0x10 ] ) == diff[ 0 ] )|| ( ( s[ i ] ^ s[ i ^ 0x20 ] ) == diff[ 0 ] )|| ( ( s[ i ] ^ s[ i ^ 0x40 ] ) == diff[ 0 ] )|| ( ( s[ i ] ^ s[ i ^ 0x80 ] ) == diff[ 0 ] ) ) {
...
int generate_test_vectors()
{
	...
	count = 0;
	unsigned char token[8] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
	unsigned char token1[254] = {
	...
	0xb0, 0xb1, 0xb2, 0xb3, 0xb4, 0xb5, 0xb6, 0xb7, 0xb8, 0xba, 0xbb, 0xbc, 0xbd, 0xbe, 0xbf,
	...
	for( pos = 0; pos < 16; ++pos ) {
		...
		for( i1 = 1; i1 < 255; ++i1 ) {
			...
			if ( faulty_aead_decrypt(msg2, &mlen2, NULL, ct1, clen, ad, adlen, nonce, key, diff, pos ) == 0 ) {
				...
				printf("\n------------------------------------Tag Compare is successful!! at the position position = (%d, %d) with input diff = 0x%02x, output diff = 0x%02x\n\n", (pos%4), (pos/4),diff,i1);
