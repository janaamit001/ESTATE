...
void faulty_sub_bytes(u8 *state_bytes, unsigned char diff, unsigned char pos)
{
	//copy state
	for (u8 i=0; i<16; i++)
		st_sb[i] = state_bytes[i];
	
	//inject fault 'diff' at the positin 'pos' 
	state_bytes[pos] ^= diff;	
...
void faulty_tweaes_enc(u8 *ct, const u8 (*round_keys)[16], const u8 *twk, const u8 *pt, unsigned char diff, unsigned char pos)
{
    u8 exp_twk[CRYPTO_EXPTWEAKBITS] = { 0 };
    ...
    // last round
    faulty_sub_bytes(&ct[0], diff, pos );
    shift_rows(&ct[0]);
    add_round_key(&ct[0], round_keys[10]);
}
